...
│  • Remove non-printable ASCII (< 0x20 except whitespace)           │
│  • NFKC normalization (ﬃ→ffi, Ｇ→G, ①→1)                          │
└─────────────────────────┬───────────────────────────────────────────┘
                          ▼
┌─── STEP 2: HOST NORMALIZATION ──────────────────────────────────────┐
│  • Multi-pass percent decode (up to 5 iterations)                   │
│    %25%32%46 → %2F → /                                             │
│  • Strip invisible chars from hostname                              │
│  • NFKC normalization on hostname                                   │
│  • IDNA/Punycode encoding (Unicode → xn-- ASCII form)              │
...
│  • Collapse redundant // → /                                        │
│  • Resolve traversal: /../ and /./  (posixpath.normpath)            │
│  • Ensure leading /                                                 │
└─────────────────────────┬───────────────────────────────────────────┘
                          ▼
┌─── STEP 4: BLOB MASKING ────────────────────────────────────────────┐
│  • Detect hex strings ≥32 chars → replace with [HEX_BLOB] tag      │
│  • Detect base64 strings ≥40 chars → replace with [BASE64_BLOB]    │
│  • Applied to both path segments and query parameter values         │
└─────────────────────────┬───────────────────────────────────────────┘
                          ▼
┌─── STEP 5: QUERY CANONICALIZATION ──────────────────────────────────┐
│  • Multi-pass percent decode each value (5 iterations)              │
│  • Remove tracking params (utm_*, ref, fbclid, etc.)                │
│  • Mask hex/base64 blobs in values                                  │
│  • Sort remaining params alphabetically by key                      │
│  • Re-encode with urlencode(sorted(...))                            │
└─────────────────────────┬───────────────────────────────────────────┘
                          ▼
┌─── STEP 6: PORT STRIPPING ──────────────────────────────────────────┐
│  • Drop default ports: :80 (HTTP), :443 (HTTPS), :21 (FTP)         │
│  • Keep non-standard ports (e.g., :8080, :4443)                     │
└─────────────────────────┬───────────────────────────────────────────┘
                          ▼
┌─── STEP 7: ASSEMBLE canonical_url ──────────────────────────────────┐
│  scheme + "://" + normalized_host [+ ":port"] + masked_path         │
│  [+ "?" + canonical_query]                                          │
└─────────────────────────────────────────────────────────────────────┘
```
---
### All 7 Attack Types: Raw → canonical_url Transformation

| # | Attack Type | Raw URL (Input) | canonical_url (Output) | What Changed |
|---|------------|----------------|----------------------|-------------|
| 1 | ✅ Clean | `https://www.google.com/search?q=weather` | `https://www.google.com/search?q=weather` | Nothing — already canonical |
| 2 | ❌ Encoded | `https://example.com/%E2%80%8B%2F%61%64%6D%69%6E` | `https://example.com/admin` | Multi-pass decode: ZWS stripped, hex→ASCII |
| 3 | ❌ Unicode | `https://аpple.com/login` | `https://xn--pple-43d.com/login` | Cyrillic а → IDNA Punycode `xn--pple-43d` |
| 4 | ❌ IP Obfus | `http://0x7f000001/phish` | `http://127.0.0.1/phish` | Hex IP canonicalized to dotted decimal |
| 5 | ❌ IDN | `https://xn--pple-43d.com/login` | `https://xn--pple-43d.com/login` | Already Punycode — kept as-is |
| 6 | ❌ Invisible | `https://exam​ple.com/page` | `https://example.com/page` | Zero-width space (U+200B) stripped |
| 7 | ❌ Mixed | `https://gooɡle.com/search` | `https://xn--goole-qx5a.com/search` | IPA ɡ (U+0261) → IDNA Punycode |
