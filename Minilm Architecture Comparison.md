...
| **GLUE Avg** | 78.4 | 82.9 | 77.0 | 77.7 | 83.5 |
...
| **AUC-ROC** | Discriminative ability across all thresholds | **98.91%** | — | Model capacity proof |
...
### End-to-End Preprocessing Architecture
...
│  ├── NFKC normalization (canonical form)                     │
│  ├── Multi-pass percent decoding                             │
│  │   %E2%80%8B → ​ (zero-width space) → stripped             │
│  ├── Invisible character stripping                           │
│  │   Remove ZWJ, ZWNJ, RTL/LTR marks, zero-width chars      │
│  ├── Confusable detection (Unicode TR39)                     │
│  │   Cyrillic 'а' flagged as confusable with Latin 'a'       │
│  ├── IDN/Punycode resolution                                 │
│  │   xn--pple-43d.com → decoded and analyzed                 │
│  └── Homograph risk scoring                                  │
│      Mixed-script analysis → risk score 0.0–1.0              │
├──────────────────────────────────────────────────────────────┤
│  STEP 2: URL PARSING & VALIDATION                            │
│  ├── Scheme extraction & family classification               │
│  │   http / https / ftp / data / javascript / blob           │
│  ├── Host normalization                                      │
│  │   IP detection (IPv4/v6, decimal, hex, octal, private)    │
│  ├── Path canonicalization                                   │
│  │   Dot-segment removal, blob detection (hex/base64)        │
│  ├── Query parameter analysis                                │
│  │   Tracking params, encoded payloads, suspicious keys      │
│  └── Fragment analysis                                       │
│      Token keywords, embedded URLs, truncation               │
├──────────────────────────────────────────────────────────────┤
│  STEP 3: FEATURE EXTRACTION (68 Features)                    │
│  ├── TLD risk classification → HIGH / CONTEXTUAL / NORMAL    │
│  ├── Host type detection → DNS / IP4P / IP4R / IP6 / OBF     │
│  ├── Shannon entropy computation                             │
│  │   URL / Path / Query entropy → bucketed L/M/H             │
│  ├── Digit ratio & path depth → bucketed                     │
│  └── 68 rule-based phishing flags                            │
│      Severity scoring + bitmask encoding                     │
├──────────────────────────────────────────────────────────────┤
│  STEP 4: STRUCTURED TEXT BUILDER                             │
│  Feature header + canonical URL → model input                │
├──────────────────────────────────────────────────────────────┤
│  STEP 5: DEDUPLICATION & STRATIFIED SPLITTING                │
│  Composite key dedup → train/val/test (80/10/10)             │
└──────────────────────────────────────────────────────────────┘
              │
              ▼
     ┌─────────────────┐
...
### Structured Model Input Format

The preprocessed text fed to MiniLM follows a fixed-order categorical header + canonical URL:

```
PC:Phishing_URL SF:HTTPS TLD:xyz RSK:H HT:DNS IDN:0 EU:H EP:M EQ:L DRB:L DPB:M
RSF:5 RSB:H FLG:SUSP,REDIR,LONGP,HEXB,IPADR FLG_TR:0 FRU:0 FRTK:1 FRB:0 FRT:0
URL:https://evil-bank-login.xyz/verify/account/secure/login.php?token=abc123
```

| Tag | Meaning | Example Values |
|-----|---------|---------------|
| `PC` | Primary threat category | `Phishing_URL`, `IP_Address_URL`, `None` |
| `SF` | Scheme family | `HTTPS`, `HTTP`, `DATA`, `JS` |
| `TLD` / `RSK` | Top-level domain + risk level | `xyz` / `H` (high risk TLD) |
| `HT` | Host type classification | `DNS`, `IP4P` (public IPv4), `OBF` (obfuscated) |
| `IDN` | Internationalized domain flag | `0` (ASCII only) or `1` (non-ASCII present) |
| `EU/EP/EQ` | Entropy buckets (URL/Path/Query) | `L`/`M`/`H` — high entropy = suspicious randomness |
| `DRB` | Digit ratio bucket | `L`/`M`/`H` — high digits = suspicious encoded payloads |
| `DPB` | Path depth bucket | `S`/`M`/`D` — deep paths common in phishing |
| `RSF/RSB` | Rule flag count + severity bucket | `5`/`H` = 5 active flags, high severity |
| `FLG` | Active detection flags (short codes) | Up to 8 flags from 68-flag taxonomy |
| `URL` | Shortened canonical URL | The actual URL (max ~400 characters) |

### Unicode URL Attack Handling

| Attack Type | Raw Input | After Preprocessing | Detection |
|------------|-----------|-------------------|-----------|
| **Homograph** | `https://аpple.com` (Cyrillic а) | Flagged as confusable | `IDN:1`, homograph score computed |
| **Punycode** | `https://xn--pple-43d.com` | Decoded + analyzed | IDN flagged, domain inspected |
| **Invisible chars** | `example​.com` (zero-width space) | `example.com` | Invisible chars stripped |
| **RTL override** | `‮moc.elgoog` | Direction marks removed | Normalized to LTR |
| **Multi-encoding** | `%25%32%46admin` → `%2F` → `/admin` | Multi-pass decoded | Clean path extracted |
| **Mixed scripts** | `gooɡle.com` (Latin + IPA) | Mixed-script detected | Flag raised, risk scored |
| **Hex IP** | `0x7f000001` | Identified as `127.0.0.1` | `HT:OBF` (obfuscated) |
| **Decimal IP** | `2130706433` | Converted to `127.0.0.1` | `HT:IP4R` (private IPv4) |
| **Octal IP** | `0177.0.0.01` | Expanded to `127.0.0.1` | `HT:IP4R` |
...
| **41.8M URL Scale** (High) | ✅ 16 GB | ⚠️ 48+ GB | ✅ 24 GB | ✅ 12 GB | ⚠️ 32+ GB |
